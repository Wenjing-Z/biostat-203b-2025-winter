...
# Write the dataset in Parquet format into a directory (here "labevents_parquet")
write_dataset(ds_csv, path = "labevents_parquet", format = "parquet")

# You can check the total size of the Parquet file(s) from the command line:
# (Run this command in your shell; for example, in a terminal you might do:)
# du -sh labevents_parquet
#
# For example, you might see:
# 1.2G    labevents_parquet
#
# (Your actual size may vary.)

# ----- Step 2. Ingest, Select, and Filter the Parquet Data -----
start_time <- Sys.time()

# Open the Parquet dataset
ds_parquet <- open_dataset("labevents_parquet", format = "parquet")

# Select the desired columns and filter for specific lab items
result_parquet <- ds_parquet %>%
  select(subject_id, itemid, charttime, valuenum) %>%
  filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>%
  collect()  # Brings the filtered data into an R tibble
...
# Display the number of rows (data rows only)
