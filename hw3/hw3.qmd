...
write_dataset(ds_csv, path = "labevents_pq", format = "parquet")

ds_parquet <- open_dataset("labevents_pq", format = "parquet")

result_parquet <- ds_parquet %>%
  select(subject_id, itemid, charttime, valuenum) %>%
  filter(itemid %in% dlabitems_table$itemid)
  collect()  

file_path <- "labevents_pq"
...
# 使用 head() 获取前十行，并用 collect() 实际加载数据
head_data <- labevents %>% head(10) %>% collect()
