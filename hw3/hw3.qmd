...
labevents_tble <- fread("labevents.csv")
# labevents_file_path <- "labevents.csv"
# labevents_tble <- read_csv(labevents_file_path)

# Load the icustays data
icustays_file_path <- "~/mimic/icu/icustays.csv.gz"
icustays_tble <- read_csv(icustays_file_path)

# Load the lab items dictionary to get itemids for the required tests
labitems_file_path <- "~/mimic/hosp/d_labitems.csv.gz"
d_labitems_tble <- read_csv(labitems_file_path)

```


```{r}
# Filter the lab items for the required tests
# Specify the itemids for the required lab tests
required_itemids <- c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)

# Filter labevents for the required items
filtered_labevents <- labevents_tble %>%
  filter(itemid %in% required_itemids)
```
...
```{r}
# Merge with icustays_tble to ensure the measurements are only for ICU patients
merged_data <- filtered_labevents %>%
  inner_join(icustays_tble, by = "subject_id") %>%
  filter(storetime <= admittime)  # Ensure the measurement is before or during the ICU stay

```


```{r}
# Get the last available measurement for each ICU stay:
# Convert storetime to datetime for comparison
merged_data$storetime <- as.POSIXct(merged_data$storetime)

# Find the last available measurement before the ICU stay for each subject_id and hadm_id
last_measurements <- merged_data %>%
...
# Reshape the data to get one row per ICU stay with columns for each lab measurement:

# Reshape the data: one row per ICU stay with columns for each lab measurement
final_data <- last_measurements %>%
